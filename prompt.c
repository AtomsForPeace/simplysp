...
typedef struct lval {
	...
	/* Function type uses above typedef */
  	lbuiltin fun;
...
lval* lval_err(char* fmt, ...) {
	...
	/* Reallocate the actual amount of bytes used */
...
void lval_del(lval* v) {
	switch (v->type) {
		/* Do nothing special for number type and function type */
		case LVAL_NUM:
		case LVAL_FUN: 
			break;
...
lval* lval_copy(lval* v) {
	...
	switch (v->type) {
		/* Copy simple reassigns for functions and numbers */
		case LVAL_FUN:
			x->fun = v->fun;
			break;
		case LVAL_NUM:
		...
		/* Copy lists by iterating over them */
		case LVAL_SEXPR:
		case LVAL_QEXPR:
			x->count = v->count;
...
lval* builtin_head(lenv* e, lval* a) {
	...
	/* All good, take first argument */
	lval* v = lval_take(a, 0);
	
	/* Delete all elements that are not head and return head */
	while (v->count > 1) {
...
lval* builtin_tail(lenv* e, lval* a) {
	...
	/* Delete the first element and return the rest */
...
lval* builtin_op(lenv* e, lval* a, char* op) {
	...
	for (int i; i < a->count; i++) {
		...
		{
			lval_del(a);
			return lval_err("Cannot operate on a non-number! Got %s, expected %s", ltype_name(a->cell[i]->type), ltype_name(LVAL_NUM));
		}
	}
...
lval* builtin_def(lenv* e, lval* a) {
	...
	LASSERT(
		...
		"Function 'def' received incorrect number of values to symbols! Got %i, expected %i",
		syms->count,
		a->count - 1
	);
...
/* Functions to create and delete environment */
lenv* lenv_new(void) {
...
lval* lenv_get(lenv* e, lval* k) {
	/*Iterate over all enviroment items */
	for (int i = 0; i < e->count; i++) {
...
void lenv_put(lenv* e, lval* k, lval* v) {
	/* Iterate over all environment items and see if variable already exists */
	for (int i = 0; i < e->count; i++) {
		/* If variable exists, delete and replace with user given variable */
		if (strcmp(e->syms[i], k->sym) == 0) {
			lval_del(e->vals[i]);
	...
	/* If no existing variable found, create new */
	e->count++;
	e->vals = realloc(e->vals, sizeof(lval*) * e->count);
	e->syms = realloc(e->syms, sizeof(char*) * e->count);

	/* Copy contents of lval and symbol string into new lcoation */
...
int main(int argc, char** argv) {
	...
	mpca_lang(MPCA_LANG_DEFAULT,
		"                                                       \
			...
			simplysp : /^/ <expr>* /$/ ;             			\
		",
		Number, Symbol, Sexpr, Qexpr, Expr, Simplysp);
	...
	lenv_del(e);
	mpc_cleanup(5, Number, Symbol, Sexpr, Qexpr, Expr, Simplysp);
